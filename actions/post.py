...
from core.utils import TWEET_MAX_LENGTH, build_header, count_tweet_length, split_program_block
from dotenv import load_dotenv


@dataclass
class RateLimitState:
    """1回の投稿セッション中のレート制限情報を保持する"""
    remaining: int | None = None
    reset: int | None = None


def update_rate_limit_from_response(response, logger, state: RateLimitState):
    ...
    try:
        ...
        if hasattr(response, 'resp') and hasattr(response.resp, 'headers'):
            ...
            if remaining is not None and reset is not None:
                ...
                logger.debug(f"レート制限情報更新 (Header): 残り={state.remaining}, 上限={limit_val}, リセット={datetime.fromtimestamp(state.reset, tz=timezone.utc)}")
                return True

        # v2 レスポンスの rate_limit 属性
        elif hasattr(response, 'rate_limit') and response.rate_limit is not None:
            state.remaining = response.rate_limit.remaining
            state.reset = response.rate_limit.reset
            limit = response.rate_limit.limit
            logger.debug(f"レート制限情報更新 (v2 response): 残り={state.remaining}, 上限={limit}, リセット={datetime.fromtimestamp(state.reset, tz=timezone.utc)}")
            return True
...
def post_tweet_with_retry(client, text, in_reply_to_tweet_id, logger, state: RateLimitState, max_retries=3, base_delay=10):
    """ツイート投稿関数 (リトライ、レート制限考慮)"""
    for attempt in range(max_retries):
        try:
            # 事前待機チェック
            if state.remaining is not None and state.remaining <= 1:
                if state.reset is not None and state.reset > 0:
                    wait_time = datetime.fromtimestamp(state.reset, tz=timezone.utc) - datetime.now(timezone.utc)
            ...
            update_rate_limit_from_response(response, logger, state)
            return tweet_id
            ...
            if hasattr(e, 'response') and e.response is not None and hasattr(e.response, 'headers'):
                ...
                if reset_header:
                    try:
                        ...
                        logger.info(f"レートリミットリセット時刻 (ヘッダーより): {datetime.fromtimestamp(reset_time, tz=timezone.utc)}")
                    except ValueError:
                        pass

            if reset_time:
                wait_time = datetime.fromtimestamp(reset_time, tz=timezone.utc) - datetime.now(timezone.utc)
...
def run_post(target_date_str: str):
    ...
    # ファイルの読み込み（gather後に手編集された内容もそのまま反映する）
